// gpunet-turn — a bundled TURN server with two operating modes.
//
// SIDECAR MODE (default when --auth-secret is given):
//   Started by gpunet-node on relay-capable rigs.  Takes the per-rig
//   secret over the command line and runs a dumb TURN forwarder.
//
//     gpunet-turn --auth-secret=<hex> [--listen=…] [--realm=…] [--external-ip=…]
//
...
//
//     gpunet-turn --server=https://pool.example.com --token=<pair-token>
//
// Provision flow for a dedicated relay box:
//   1. Operator generates a one-liner / pair token from the dashboard.
//   2. SSH to the relay, run `gpunet-turn --server=… --token=…`.
...
func main() {
	...
	if *server != "" {
		if *secret != "" {
			fmt.Fprintln(os.Stderr,
				"gpunet-turn: --auth-secret and --server are mutually exclusive")
			os.Exit(2)
		}
	...
	if *secret == "" {
		fmt.Fprintln(os.Stderr,
			"gpunet-turn: pass either --auth-secret (sidecar) or --server (standalone)")
		flag.Usage()
		os.Exit(2)
	}
...
func runSidecar(listenAddr, realm, secret, extIP string) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "gpunet-turn: %v\n", err)
		os.Exit(1)
	}
	logListeners(conns, realm, extIP)
	waitForExit()
	if err := srv.Close(); err != nil {
		fmt.Fprintf(os.Stderr, "gpunet-turn: close: %v\n", err)
	}
}
...
func runStandalone(serverURL, token, stateDir, listenAddr, realmFlag, extIP,
	...
	if err := os.MkdirAll(stateDir, 0o700); err != nil {
		fmt.Fprintf(os.Stderr, "gpunet-turn: cannot create state-dir %s: %v\n",
			stateDir, err)
		os.Exit(1)
	}
	...
	if extIP == "" {
		if ip := discoverPublicIP(3 * time.Second); ip != "" {
			...
			fmt.Fprintf(os.Stderr,
				"gpunet-turn: discovered public_ip=%s via STUN\n", extIP)
		} else {
			fmt.Fprintln(os.Stderr,
				"gpunet-turn: warning — could not auto-discover public IP; "+
	...
	for {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "gpunet-turn: cycle failed: %v\n", err)
...
func runOnePairCycle(ctx context.Context, cfg pairConfig,
	...
	fmt.Fprintf(os.Stderr,
		"gpunet-turn: registered as agent_id=%s, advertising coturn_port=%d\n",
		cfg.AgentID, port)
...
func startTurn(listenAddr, realm, secret, extIP string,
	...
	if wildcard {
		if pc, err := net.ListenPacket("udp4",
			...
			fmt.Fprintf(os.Stderr, "gpunet-turn: udp4 bind skipped: %v\n", err)
		}
		if pc, err := net.ListenPacket("udp6",
			net.JoinHostPort("::", portStr)); err == nil {
			packetConns = append(packetConns, pc)
		} else {
			fmt.Fprintf(os.Stderr, "gpunet-turn: udp6 bind skipped: %v\n", err)
		}
	} else {
...
func logListeners(conns []net.PacketConn, realm, extIP string) {
	for _, pc := range conns {
		fmt.Fprintf(os.Stderr, "gpunet-turn: listening on %s (%s) realm=%s\n",
			pc.LocalAddr(), pc.LocalAddr().Network(), realm)
	}
	if extIP != "" {
		fmt.Fprintf(os.Stderr, "gpunet-turn: external-ip=%s\n", extIP)
	}
}
...
func defaultStateDir() string {
	if d, err := os.UserConfigDir(); err == nil && d != "" {
		return filepath.Join(d, "gpunet-turn")
	}
	if runtime.GOOS == "windows" {
		return filepath.Join(os.Getenv("APPDATA"), "gpunet-turn")
	}
	return filepath.Join(os.Getenv("HOME"), ".gpunet-turn")
}
