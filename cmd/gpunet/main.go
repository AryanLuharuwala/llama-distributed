// gpunet — the user-facing CLI for the distributed inference network.
//
//	curl -fsSL https://<host>/install.sh | sh   # one-line installer
//	gpunet login                                  # browser-paired device code
//	gpunet connect                                # bring the rig online
//
// All commands accept --json so the desktop widget can parse them
// programmatically.  No daemon required for the common case: `gpunet connect`
// runs gpunet-node in the foreground until Ctrl-C; daemon mode is opt-in via
...
const usage = `gpunet — distributed inference rig control

USAGE
  gpunet <command> [flags]

COMMANDS
  login      Pair this machine with your account (device-code in browser)
  connect    Bring the rig online (runs gpunet-node in foreground)
  status     Show local rig state
  stop       Tell a running gpunet connect to shut down cleanly
...
func credsPath() string {
	// gpunet rename: prefer GPUNET_CREDS / ~/.config/gpunet; fall back to the
	// legacy SURD_CREDS / ~/.config/gpunet so an already-logged-in user keeps
	// their token across the rename.
	if c := os.Getenv("GPUNET_CREDS"); c != "" {
		return c
	}
	if c := os.Getenv("SURD_CREDS"); c != "" {
		return c
	}
	sub := func(dir string) (cur, legacy string) {
		return filepath.Join(dir, "gpunet", "credentials.json"),
			filepath.Join(dir, "surd", "credentials.json")
	}
	var base string
	...
	cur, legacy := sub(base)
	// Use the legacy path only if it exists and the new one doesn't yet.
...
func main() {
	...
	case "version", "-v", "--version":
		fmt.Println("gpunet", version)
	case "help", "-h", "--help":
		fmt.Print(usage)
	default:
		fmt.Fprintf(os.Stderr, "gpunet: unknown command %q\n\n%s", cmd, usage)
		os.Exit(2)
	}
	if err != nil {
		fmt.Fprintln(os.Stderr, "gpunet:", err)
		os.Exit(1)
	}
}
...
func cmdLogin(args []string) error {
	...
	for time.Now().Before(deadline) {
		...
		// tok.Server is a WS URL, gpunet works at the HTTP layer so we
		...
		if *jsonOut {
			...
			fmt.Println()
			fmt.Println("    next:  gpunet connect")
		}
		return nil
	}
	return errors.New("login timed out — run `gpunet login` again")
}
...
// and execs `gpunet-node` with the resulting flags.  Foreground; Ctrl-C
// forwards SIGTERM to gpunet-node for a clean unregister.
func cmdConnect(args []string) error {
	fs := flag.NewFlagSet("connect", flag.ExitOnError)
	jsonOut := fs.Bool("json", false, "machine-readable status")
	binary := fs.String("gpunet-node", "gpunet-node", "path to the gpunet-node binary")
	...
	dryRun := fs.Bool("dry-run", false, "print the gpunet-node command and exit")
	fs.Parse(args)

	creds, err := loadCreds()
	if err != nil {
		return errors.New("no credentials — run `gpunet login` first")
	}
	...
	if *inviteToken != "" {
		if err := redeemInvite(creds, *inviteToken); err != nil {
			fmt.Fprintln(os.Stderr, "gpunet: warning — invite redeem failed:", err)
		}
	}
	...
	// Build gpunet-node argv.  Keep this minimal — gpunet-node has its own
	// config file for the long tail; gpunet just hands over identity + server.
	argv := []string{
	...
	// Exec gpunet-node, forwarding signals.
	...
	cmd.Stdin = os.Stdin
	// Record our PID so `gpunet stop` and the widget can find us.  Best-effort
	...
	if err := cmd.Run(); err != nil {
		...
		// Common case: gpunet-node not on PATH.
		if errors.Is(err, exec.ErrNotFound) {
			return fmt.Errorf("gpunet-node binary not found on PATH (set --gpunet-node=…)")
		}
		return fmt.Errorf("gpunet-node exited: %w", err)
	}
	return nil
}
...
func redeemInvite(c *credentials, token string) error {
	...
	// the user can also redeem via the web UI before running `gpunet connect`.
...
func cmdStatus(args []string) error {
	...
	if err != nil {
		...
		fmt.Println("not logged in — run `gpunet login`")
		return nil
	}

	// Local state: is gpunet connect running?
	running := false
...
// readControlPID returns the PID of a running `gpunet connect`, if any.
...
// controlPIDPath is where `gpunet connect` parks its PID for `gpunet stop`
...
func cmdStop(args []string) error {
	...
	if err != nil {
		...
		if !*jsonOut {
			fmt.Println("no running gpunet connect found.")
		}
		return nil
	}
	pid := 0
	...
	// SIGTERM gives gpunet-node time to unregister cleanly.  On Windows
	...
	if err != nil {
		...
		if !*jsonOut {
			fmt.Fprintln(os.Stderr, "gpunet: signal pid", pid, ":", err)
		}
		return err
	}
	report("stopped", map[string]any{"pid": pid})
	if !*jsonOut {
		fmt.Println("  ✓ sent SIGTERM to gpunet connect (pid", pid, ")")
	}
	return nil
}
...
func cmdPools(args []string) error {
	...
	creds, err := loadCreds()
	if err != nil {
		return errors.New("not logged in — run `gpunet login`")
	}
	req, _ := http.NewRequest("GET",
...
// ── daemon ───────────────────────────────────────────────────────────────
// `gpunet daemon install|uninstall|status` registers `gpunet connect` as a
// user-level service that auto-starts at login.  Per-platform backend:
//
//   linux   — systemd user unit at ~/.config/systemd/user/gpunet.service
//   darwin  — launchd LaunchAgent at ~/Library/LaunchAgents/com.gpunet.connect.plist
//   windows — scheduled task gpunet-connect via schtasks
//
...
const (
	daemonServiceID  = "com.gpunet.connect"
	daemonLabelShort = "gpunet-connect"
)
...
func cmdDaemon(args []string) error {
	...
	case "help", "-h", "--help":
		...
		return nil
	default:
		fmt.Fprintln(os.Stderr, "gpunet daemon: unknown subcommand", sub)
...
const daemonUsage = `gpunet daemon — auto-start gpunet connect at login

USAGE
  gpunet daemon install    register and start the service
  gpunet daemon uninstall  stop and unregister
  gpunet daemon status     show whether the service is running
`

// gpunetBinaryPath resolves to the absolute path of the running binary so
// the service definition survives PATH changes.
func gpunetBinaryPath() (string, error) {
...
Description=gpunet — distributed inference rig
...
`, exe)
	...
	fmt.Println("  ✓ gpunet is now running as a user systemd service")
...
func launchdInstall(exe string) error {
	home, _ := os.UserHomeDir()
	logDir := filepath.Join(home, "Library", "Logs", "gpunet")
...
<dict>
  ...
  <key>StandardOutPath</key><string>%s/gpunet.out.log</string>
  <key>StandardErrorPath</key><string>%s/gpunet.err.log</string>
</dict>
</plist>
`, daemonServiceID, exe, logDir, logDir)
	...
	fmt.Println("  ✓ gpunet LaunchAgent installed and started")
...
func windowsTaskInstall(exe string) error {
	...
	if err := runStream("schtasks", "/Run", "/TN", daemonLabelShort); err != nil {
		...
		fmt.Fprintln(os.Stderr, "gpunet: warning — schtasks /Run failed:", err)
	}
...
func defaultServer() string {
	if s := os.Getenv("GPUNET_SERVER"); s != "" {
		return s
	}
